...
def _run_bootstrap(
    ...
    # Pass Conan toolchain if it exists (search common generator locations)
    conan_toolchain = build_dir / "conan_toolchain.cmake"
    if not conan_toolchain.exists():
        # Conan may place the toolchain under generators/ or nested build-type dirs
        for candidate in build_dir.glob("**/conan_toolchain.cmake"):
    ...
    if conan_toolchain.exists():
        ...
        logger.warning("Conan toolchain file not found; CMake will run without it")
...
def _generate_conan_profile(
    ...
    if conan_os == "Windows":
        ...
        # Determine if using LLVM/Clang or actual MSVC
        is_llvm_toolchain = vendor.lower() in ("llvm", "clang")

        if is_llvm_toolchain:
            # For LLVM/Clang on Windows, use clang profile to avoid MSVC-specific flags
            # This may require building some packages from source
            conan_compiler = "clang"
            # Extract major version from toolchain name
            version_parts = version.split(".")
            compiler_version = version_parts[0] if version_parts else "18"

            # Determine compiler executables for Windows LLVM
            if toolchain_path:
                bin_dir = toolchain_path / "bin"
                c_compiler = bin_dir / "clang.exe"
                cxx_compiler = bin_dir / "clang++.exe"
                rc_compiler = bin_dir / "llvm-rc.exe"

                c_compiler_str = str(c_compiler).replace("\\", "/")
                cxx_compiler_str = str(cxx_compiler).replace("\\", "/")
                rc_compiler_str = str(rc_compiler).replace("\\", "/")
            else:
                c_compiler_str = "clang"
                cxx_compiler_str = "clang++"
                rc_compiler_str = "llvm-rc"

            # Generate Windows Clang profile with compiler paths
            # Note: Some packages may not have prebuilt binaries for clang on Windows
            profile_content = f"""# Conan profile generated by ToolchainKit
# Toolchain: {toolchain_name} (using Clang profile)
# Generated for: {platform_str}
# Note: Some packages may need to be built from source (--build=missing)
...
tools.cmake.cmaketoolchain:extra_variables={{"CMAKE_RC_COMPILER": "{rc_compiler_str}"}}
"""
        else:
            # For actual MSVC toolchain
            conan_compiler = "msvc"
            compiler_version = "193"  # Visual Studio 2022

            # Generate Windows MSVC profile
            profile_content = f"""# Conan profile generated by ToolchainKit
# Toolchain: {toolchain_name} (using MSVC profile)
# Generated for: {platform_str}
